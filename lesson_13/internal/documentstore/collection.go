...
func (s *Collection) Put(doc Document) error {
	...
	if existed {
		...
		if old != nil {
			if err := s.removeFromIndexesLocked(key, old); err != nil {
				return err
			}
		}
	} else {
	...
	if err := s.addToIndexesLocked(key, &doc); err != nil {
		return err
	}
...
func (s *Collection) Delete(key string) bool {
	...
	_ = s.removeFromIndexesLocked(key, old)
	delete(s.Items, key)
...
// --- Locked versions: call ONLY with s.mu already held (Lock or RLock where safe) ---

func (s *Collection) removeFromIndexesLocked(docKey string, doc *Document) error {
	if s.indexes == nil {
		return nil
	}
...
func (s *Collection) addToIndexesLocked(docKey string, doc *Document) error {
	if s.indexes == nil {
		return nil
	}
	for fieldName, tree := range s.indexes {
