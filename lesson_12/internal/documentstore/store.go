...
func NewStore() *Store {
	gob.Register(&Collection{}) // need for cases when we dumb/restore the Store object, gob needs concrete type for Collectable
...
func (s *Store) DumpToFile(filename string) error {
	...
	cleanup := true // cleanup only on failure (atomic rename removes tmpName)
	defer func() {
		_ = tmp.Close()
		if cleanup {
			_ = os.Remove(tmpName)
